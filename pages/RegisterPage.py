...
class RegisterPage(BasePage):
    ...
    yes_redio_button_xpath = "//input[@name='newsletter'][@value='1']"
    ...
    def enter_password_confirm(self, conf_password_text):
        self.type_into_element(conf_password_text, "confirm_password_field_id", self.confirm_password_field_id)
    ...
    def select_yes_radio_button(self):
        self.element_click("yes_redio_button_xpath", self.yes_redio_button_xpath)

    def register_an_account(self, first_name_text, last_name_text, email_text, telephone_text, password_text, conf_password_text, yes_or_no, privacy_policy):
        ...
        self.enter_password_confirm(conf_password_text)
    ...
    def retrieve_email_warning_(self):
        return self.retrieve_element_text("email_warning_xpath", self.email_warning_xpath)

    def retrieve_telephone_warning_(self):
        return self.retrieve_element_text("telephone_warning_xpath", self.telephone_warning_xpath)

    def retrieve_password_warning_(self):
        return self.retrieve_element_text("password_warning_xpath", self.password_warning_xpath)

    def verify_all_warnings(self, expected_privacy_warning_text, expected_warning_message, expected_warning_message_last, expected_email_warning, expected_telephone_warning, expected_password_warning):
        actual_privacy_warning_text = self.retrieve_privacy_policy_warning()
        actual_warning_message = self.retrieve_first_name_warning()
        actual_warning_message_last = self.retrieve_last_name_warning()
        actual_email_warning = self.retrieve_email_warning_()
        actual_telephone_warning = self.retrieve_telephone_warning_()
        actual_password_warning = self.retrieve_password_warning_()
        ...
        if expected_privacy_warning_text.__contains__(actual_privacy_warning_text):
            if expected_warning_message.__eq__(actual_warning_message):
                if expected_warning_message_last.__eq__(actual_warning_message_last):
